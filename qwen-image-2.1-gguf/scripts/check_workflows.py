...
def main():
    ...
    for path in sorted((ROOT/'workflows').glob('*.json')):
        ...
        assert api['1']['class_type']=='UnetLoaderGGUF'
        assert api['1']['inputs']['unet_name']=='qwen-image-2.1-Q4_K_M.gguf'
        if path.name.startswith(('06-', '07-')):
            assert api['2']['inputs']['device']=='cpu'
            assert api['7']['class_type']=='VAEDecodeTiled'
        ks=api['6']['inputs']
        ...
        count+=1
    assert count==7
    print('PASS: 7 UI/API pairs, widget values, links, dimensions and edit routing')
