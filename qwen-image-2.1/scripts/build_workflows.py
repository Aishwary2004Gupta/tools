...
MODELS = ['qwen_image_2.1_int8_convrot.safetensors', 'qwen3vl_8b_int8_convrot.safetensors', 'qwen_image_2.1_vae_bf16.safetensors']
...
def graph(mode='t2i', width=1024, height=1024, steps=40):
    g = {
        '1': {'class_type':'UNETLoader','inputs':{'unet_name':MODELS[0],'weight_dtype':'default'}},
        ...
        '8': {'class_type':'SaveImage','inputs':{'images':['7',0],'filename_prefix':'Qwen21/'+mode}},
    }
...
OUTPUTS = {'UNETLoader':[('MODEL','MODEL')], 'CLIPLoader':[('CLIP','CLIP')], 'VAELoader':[('VAE','VAE')], 'TextEncodeQwenImage21':[('positive','CONDITIONING'),('negative','CONDITIONING'),('latent','LATENT')], 'EmptyLatentImage':[('LATENT','LATENT')], 'KSampler':[('LATENT','LATENT')], 'VAEDecode':[('IMAGE','IMAGE')], 'SaveImage':[('images','IMAGE')], 'LoadImage':[('IMAGE','IMAGE'),('MASK','MASK')], 'QwenImage21Cache':[('MODEL','MODEL')]}
WIDGETS = {'UNETLoader':['unet_name','weight_dtype'], 'CLIPLoader':['clip_name','type','device'], 'VAELoader':['vae_name'], 'TextEncodeQwenImage21':['prompt','negative_prompt','resolution'], 'EmptyLatentImage':['width','height','batch_size'], 'KSampler':['seed','control_after_generate','steps','cfg','sampler_name','scheduler','denoise'], 'SaveImage':['filename_prefix'], 'LoadImage':['image','upload'], 'QwenImage21Cache':['device','dtype'], 'VAEDecode':[]}
...
SPECS=[('01-text-to-image','t2i',1024,1024,40),('02-text-to-image-2k','t2i',2048,1152,50),('03-image-edit','edit',1024,1024,40),('04-transparent-rgba','rgba',1024,1024,40),('05-two-reference-edit','references',1024,1024,40)]
...
def main():
    ...
    print('Built 5 UI workflows and 5 API graphs.')
